<?php 
/**
 * Admin Page Class for Fixed Circular Navigation Menu
 *
 * This class is derived and customized from Admin-Page-Class (https://github.com/bainternet/Admin-Page-Class script) which is 
 * a class for creating custom options panels for WordPress. 
 * 
 * @author    Enrique Crespo Molera <contact@e-crespo.com>
 * 
 * @original_author Ohad Raz (email: admin@bainternet.info)
 ...
 * 
 * @Last revised on 9th September 2014 
 */

if ( ! class_exists( 'BF_Admin_Circular_Nav') ) :
 ...
 */
  ...
  class BF_Admin_Circular_Nav {
    ...
    public function AddMenuTopPage() {
      ...
      $id = add_menu_page($this->args['page_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array($this, 'DisplayPage'), $this->args['icon_url'], $this->args['position']);
      $page = add_submenu_page($id, $this->args['page_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array($this, 'DisplayPage'));
      if ($page){
         $this->_Slug = $page;
    ...
    public function plupload_head_js(){
      if ($this->has_field('plupload')){
         $plupload_init = array(
            ...
            'filters'             => array(array('title' => __('Allowed Files','acn'), 'extensions' => '*')),
            'multipart'           => true,
    ...
    public function panel_script(){
      ?>
      <script>
        /* Modal window handling for icon chooser for font-awesome integration */
        function overlay(elm){
            xyz = document.getElementById(elm);
    ...
    public function edit_insert_to_post_text( $safe_text, $text ) {
      if( is_admin() && 'Insert into Post' == $safe_text){
        if (isset($_REQUEST['apc']) && 'insert_file' == $_REQUEST['apc'] )
          return str_replace(__('Insert into Post'), __('Use this File','acn'), $safe_text);
        else
          return str_replace(__('Insert into Post'), __('Use this Image','acn'), $safe_text);
      }
      return $safe_text;
    }
    ...
    public function DisplayPage() {
      ...
      echo '<form method="post" name="'.apply_filters('apc_form_name', 'admin_page_class',$this).'" class="'.apply_filters('apc_form_class', 'admin_page_class',$this).'" id="'.apply_filters('apc_form_id', 'admin_page_class',$this).'" action="" enctype="multipart/form-data">
        ...
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" value="'.esc_attr(__('Save Changes','acn')).'" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn"><br><br>
        </div>'."\n".'
      <br style="clear:both"><br>
      </div>'."\n";
      wp_nonce_field( basename(__FILE__), 'BF_Admin_Circular_Nav_nonce' );
      ...
      if ($this->saved_flag){
        ...
        if (is_array($this->errors) && count($this->errors) > 0 ){
          ...
          echo '<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button><strong>'.__('Settings saved.','acn').'</strong></div>'."\n";
        }
        echo '</div>'."\n";
      }
        ...
        if (method_exists($this,'show_field_' . $field['type'])){
          ...
          switch($field['type']) {
            ...
            case 'TABS_Listing':
              ...
              echo '</ul>'."\n".'</div>'."\n".'<div id="sections-load"><i class="fa fa-5x fa-spinner fa-spin"></i></div><div id="sections" class="sections" style="display:none;">'."\n";
              break;
            case 'OpenTab':
              $this->tab_div = true;
    ...
    echo '</div>'."\n".'<div style="clear:both"></div>'."\n".'<div class="footer_wrap">'."\n".'
        <div style="float:right;margin:32px 0 0 0">'."\n".'
          <input type="submit" style="margin-left: 25px;" name="Submit" class="'.apply_filters('admin_page_class_submit_class', 'btn-info').' btn" value="'.esc_attr(__('Save Changes','acn')).'" />'."\n".'
          <br><br>
        </div>'."\n".'
        <br style="clear:both"><br>
      </div>'."\n";
  ...
  public function load_scripts_styles() {
    ...
    // Font-awesome support (added by e-crespo)
  ...
  /**
   * Check For select field to enqueue Select2 
   * @since 1.0.0
   * @access public
   */
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image','acn' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete' ,'acn') . "' /></a>";
  ...
  */
  public function wp_ajax_delete_image() {
    ...
    if ( $ok ){
      ...
      echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.','acn')));
      die();
    }
  }
    ...
    if ( $this->has_field_any(array('color' ,'typo' ))  && $this->is_edit_page() ) {
      if( wp_style_is( 'wp-color-picker', 'registered' ) ) {
          ...
          //wp_enqueue_script( 'wp-color-picker' );
           wp_enqueue_script( 'cn-color-picker', plugins_url( 'js/cn-color-picker.js', __FILE__ ), array( 'wp-color-picker' ), false, true );
      }else{
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
         ...
         foreach ($meta as $me){
           //for labling toggles
           $item_lab =  isset($me[$field['fields'][0]['id']])? 'Item '.$item_idx.' - '.$me[$field['fields'][0]['id']]: 'Item '.$item_idx;
           echo '<div class="at-repater-block"><strong>&nbsp;&nbsp;'.$item_lab.'</strong><br/><table class="repeater-table cn-edit-hide-element">';
           if ($field['inline']){
          ...
          foreach ($field['fields'] as $f){
            ...
            if (($field_idx == 3) || ($field_idx == 6)){
               echo '</tr>'; echo '<tr class="at-inline" VALIGN="top">';
            }
          $field_idx++;  
          }
        if ($field['inline']){  
        ...
        <span class="at-re-toggle"><img src="';
           if ($this->_Local_images){
             echo $plugin_path.'/images/edit2.png';
           }else{
             echo 'http://i.imgur.com/ka0E2.png';
           }
           echo '" alt="Edit" title="Edit" class="edit-'.$field['id'].'"/></span> 
        <img src="';
        if ($this->_Local_images){
          echo $plugin_path.'/images/remove2.png';
        }else{
          echo 'http://i.imgur.com/g8Duj.png';
        }
        echo '" alt="'.__('Remove','acn').'" title="'.__('Remove','acn').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '<img src="';
    if ($this->_Local_images){
      echo $plugin_path.'/images/add2.png';
    }else{
      echo 'http://i.imgur.com/w5Tuc.png';
    }
    echo '" alt="'.__('Add new navigation element','acn').'" title="'.__('Add new navigation element','acn').'" id="add-'.$jsid.'"><br/></div>';
    ...
    echo '</table>
        <span class="at-re-toggle"><img src="';
           ...
           echo '" alt="Edit" title="Edit" class="edit-'.$field['id'].'"/></span> <img src="';
    if ($this->_Local_images){
      echo $plugin_path.'/images/remove2.png';
    }else{
      echo 'http://i.imgur.com/g8Duj.png';
    ...
    echo '" alt="'.__('Remove','acn').'" title="'.__('Remove','acn').'" class="remove-'.$jsid.'" id="remove-'.$jsid.'"></div>'."\n";
    ...
    echo '<script>
        jQuery(document).ready(function($) {
          ...
          $(document).on("click","#add-'.$jsid.'", function() {
            ...
            $(".each-icon input:radio").off("change");
            $(".icon-edit-btn").off("click");
            $(".icon-changer-overlay").off("click");
            $(".each-icon input:radio").change(function () {
                if($(this).is(":checked")){
                    var chosenIcon = $(this).val();
                    if(chosenIcon != "none"){
                        $(this).parent().parent().parent().parent().children(".icon-selected").empty().html("<i></i>");
                        $(this).parent().parent().parent().parent().children(".icon-selected").children().removeClass().addClass("fa fa-2x fa-" + chosenIcon).attr("title","" + chosenIcon);
                    }else{
                       $(this).parent().parent().parent().parent().children(".icon-selected").empty().html("<strong>No icon</strong>");
                    }
             ...
             $(".icon-changer-overlay").click(function (){
                $(this).hide();
             });
          });
          ...
          $(document).on( "click", ".at-re-toggle", function( e ){
            var body = $(this).prev();
            if( body.is(":visible") )
                    body.slideUp("fast");
            else
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_begin( $field, $meta) {
    ...
    if ( $field['name'] != '' || $field['name'] != FALSE )
        echo "<div class='at-label ".(isset($field['class'])? " {$field['class']}": "")."'><label for='{$field['id']}'>{$field['name']}</label></div>\n";
  }
  ...
  public function show_field_plupload( $field, $meta) {  
    ...
    $html = '
        ...
        <div class="plupload-upload-uic hide-if-no-js '.$m1.'" id="'.$id.'plupload-upload-ui">
          <input id="'.$id.'plupload-browse-button" type="button" value="'.__('Select Files','acn').'" class="button" />
  ...
  public function show_field_imageradio( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        $checked = in_array( $key, $meta ); if ($checked){$checked = 'checked';}else{$checked = "unchecked";}
        echo '<label class="cn-image-radio-option '.$checked.'">';  
        echo "<input type='radio' class='at-radio".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) .  " />";
        echo '<img src="'.$value['image'].'" alt="'.$value['label'],'" title="'.$value['label'].'"/>';
        echo '<i></i>';
        echo "<span class='at-radio-label'>{$value['label']}</span>";
        echo '</label>';
      }
    $this->show_field_end( $field, $meta );
  }
  ...
  /**
   * Show Large Radio Field with modal window (for font-awesome list)
   *
   * @param string $field 
   * @param string $meta 
   * @since  1.0.0
   ...
   * @access public 
   */
  public function show_field_largeradiofa( $field, $meta ) {
    
    if ( ! is_array( $meta ) )
      $meta = (array) $meta;
    
    $icon_edit_button_text = __('Select Icon','acn');
   
    $this->show_field_begin( $field, $meta );
    
    if (!empty($meta)){
        if ($meta[0] != "none"){
            echo "<span class='icon-selected'><i class='fa fa-2x fa-{$meta[0]}' title='{$meta[0]}'></i></span>";
        }else{
            echo "<span class='icon-selected'><strong>No icon</strong></span>";
        }
    }else{
        echo "<span class='icon-selected'><strong>No icon</strong></span>";
    }
    
    $aux_field_id = str_replace('[', '', $field['id']); $aux_field_id = str_replace(']', '', $aux_field_id); $aux_field_id = str_replace('-', '', $aux_field_id);
    $aux_field_id2 = strval($aux_field_id);
    $escaped_id = htmlspecialchars(json_encode($aux_field_id2));
    
    echo '<a href="#" class="icon-edit-btn">'.$icon_edit_button_text.'</a>';
    echo '<div id="'.$aux_field_id2.'" class="icon-changer-overlay" style="display:none;">
        <div class="icon-changer-modal">
        <p><strong>Choose an icon</strong></p>';
    foreach ( $field['options'] as $key => $value ) {
        echo "<span class='each-icon'><input type='radio' class='at-radio".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}' value='{$value}'" . checked( in_array( $value, $meta ), true, false ) . " /> <span class='at-radio-label icons-fa'><i class='fa fa-lg fa-{$value}' title='{$value}'></i>".(($value == 'none')? " None": "")."</span></span>"."\n";
      }    
    echo '</div></div>';
  ...
  public function show_field_file( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
      ...
      if ( ! empty( $meta ) ) {
        $nonce = wp_create_nonce( 'at_ajax_delete' );
        echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files','acn') . '</strong></div>';
        echo '<ol class="at-upload">';
        foreach ( $meta as $att ) {
          // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
          echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete' ,'acn') . "</a>)</li>";
        }
        echo '</ol>';
      }
  ...
  public function show_field_media_manager($field,$meta){
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input class='at-delete_image_button button' type='button' rel='".$field['id']."' value='".__('Delete Image','acn')."' />"."\n";
    }else{
      ...
      $html .= "<input class='at-mm-upload_image_button button' type='button' rel='".$field['id']."' value='".__('Upload Image','acn')."' />"."\n";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='".$meta['id']."' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='".$meta['src']."' />";
      $html .= "<input class='at-delete_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".__('Delete Image','acn')."' />";
    }else{
      $html .= "<span class='mupload_img_holder'  data-wi='".$width."' data-he='".$height."'></span>";
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='' />";
      $html .= "<input class='at-upload_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".__('Upload Image','acn')."' />";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_typo( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . __( 'Select a color' ,'acn') . "'/>"."\n";
  ...
  public function show_field_color( $field, $meta ) {
    ...
    $this->show_field_begin( $field, $meta );
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      echo "<input class='cn-at-color-iris".(isset($field['class'])? " {$field['class']}": "")."' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' data-default-color='{$field['default']}' size='8' />";  
    }else{
      echo "<input class='at-color".(isset($field['class'])? " {$field['class']}": "")."' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' data-default-color='{$field['default']}' size='8' />";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'acn') . "'/>";
  ...
  public function show_field_posts($field, $meta) {
    ...
    // select
      ...
      echo "<option value='none'" . selected(in_array('none', $meta), true, false) . ">No</option>";
      foreach ($posts as $p) {
  ...
  public function show_field_WProle($field, $meta) {
    ...
    if ($names){
      ...
      if ('checkbox_list' == $options['type']) {
        if (isset($field['class']) && $field['class'] == 'no-toggle')
            echo "<label class='at-posts-checkbox-label'><input type='checkbox'  class='at-role-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='public'" . checked(in_array('public', $meta), true, false) . " />Non-logged users</label>";
          else
            echo "Non-logged users <input type='checkbox'  class='at-role-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='public'" . checked(in_array('public', $meta), true, false) . " /><br/>";
        foreach ($names as $n) {
          if (isset($field['class']) && $field['class'] == 'no-toggle')
            ...
            echo "{$n} <input type='checkbox'  class='at-role-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='$n'" . checked(in_array($n, $meta), true, false) . " /><br/>";
        }
      }
      // select
      else {
  ...
  /**
   * Save Data from page and generate dynamic CSS options file and HTML with navigation elements
   *
  ...
  public function save($repeater = false) {
    ...
    //check nonce
    if ( ! check_admin_referer( basename( __FILE__ ), 'BF_Admin_Circular_Nav_nonce') )
      return;
    ...
    foreach ( $this->_fields as $field ) {
      if(!in_array($field['type'],$skip)){
        ...
        //issue #27
        $validationClass = apply_filters('apc_validattion_class_name', 'BF_Admin_Circular_Nav_Validate',$this);
    ...
    $this->generate_options_css($this->_saved);
    $this->generate_cn_elements($this->_saved);
  }
  ...
  public function generate_options_css($newdata) {
        ...
        $css_dir = plugin_dir_path(dirname(__FILE__)) . 'public/assets/css/'; // Shorten code, save 1 call
        ...
        if ( ! $wp_filesystem->put_contents( $aq_uploads_dir . 'cn-options.css', $css, 0644) ) {
            file_put_contents( $aq_uploads_dir . 'cn-options.css', $css );
            return true;
        }
   ...
   /**
   * Generate html file with navigations elements. This file is going to be loaded dynamically
   * This is to solve a Back-forward cache bug in some browsers (Safari, IOs Safari and Android Stock browser)
   *
   * @param array $newdata 
   * @since 1.0.0
   * @author Enrique Crespo
   * @access public 
   */
  public function generate_cn_elements($newdata) {

        /** Define some vars **/
        $cn_data = $newdata;
        $uploads = wp_upload_dir();
        $views_dir = plugin_dir_path(dirname(__FILE__)) . 'public/views/'; // Shorten code, save 1 call
        ...
        /** Capture CSS output **/
        ob_start();
        require($views_dir . 'cn-elements.php');
        $html = ob_get_clean();
        ...
        WP_Filesystem();
        global $wp_filesystem;
        if ( ! $wp_filesystem->put_contents( $aq_uploads_dir . 'cn-elements.html', $html, 0644) ) {
            file_put_contents( $aq_uploads_dir . 'cn-elements.html', $html );
            return true;
        }
  ...
  public function addImageRadio($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'imageradio','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => 'Image Radio Field','options' => $options,'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  /**
   *  Add Large Radio Field to Page (for font-awesome icon list) using modal window
   *  @author Enrique Crespo
   *  @since  1.0.0
   *  @access public
   *  @param $id string field id, i.e. the meta key
   ...
   *  @param $repeater bool  is this a field inside a repeatr? true|false(default)
   */
  public function addLargeRadioFa($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'largeradiofa','id'=> $id,'std' => array(),'desc' => '','style' =>'','name' => 'Large Font-Awesome Radio Field','options' => $options,'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function addColor($id,$args,$repeater=false){
    $new_field = array('type' => 'color','id'=> $id,'std' => '','desc' => '','name' => 'ColorPicker Field', 'default' => '#ffffff');
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function addPosts($id,$options,$args,$repeater=false){
    ...
    $options = array_replace_recursive_php52($temp,$options);
  ...
  public function show_import_export(){
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.__('Export','acn').'</h3>
        <p>'. __('To export saved settings click the Export button bellow and you will get the export Code in the box bellow, which you can later use to import.','acn').'</p>
        <div class="export_code">
          <label for="export_code">'. __('Export Code','acn').'</label><br/>
          <textarea id="export_code"></textarea>        
          <input class="button" type="button" value="'. __('Get Export','acn').'" id="apc_export_b" />'.$this->create_export_download_link().'
      ...
      <div style="padding 10px;" class="apc_import"><h3>'.__('Import','acn').'</h3>
        <p>'. __('To Import saved settings paste the Export output in to the Import Code box bellow and click Import.','acn').'</p>
        <div class="import_code">
          <label for="import_code">'. __('Import Code','acn').'</label><br/>
          <textarea id="import_code"></textarea>
                  <input class="button" type="button"  value="'. __('Import','acn').'" id="apc_import_b" />
  ...
  public function export(){
    ...
    if (!isset($_GET['group'])){
      $re['err'] = __('error in ajax request! (1)','acn');
    ...
    if ($options !== false)
      ...
      $re['err'] = __('error in ajax request! (2)','acn');
  ...
  public function import(){
    ...
    if (!isset($_POST['imp'])){
      $re['err'] = __('error in ajax request! (3)','acn');
    ...
    if (is_array($import_code)){
      ...
      $re['success']= __('Setting imported, make sure you ','acn'). '<input class="button-primary" type="button"  value="'. __('Refresh this page','acn').'" id="apc_refresh_page_b" />';
    }else{
      $re['err'] = __('Could not import settings! (4)','acn');
    }
    //update_nonce
      $re['nonce'] = wp_create_nonce("apc_import");
    echo json_encode($re);
  ...
  public function download_file($content = null, $file_name = null){
    ...
    if ( !current_user_can('edit_themes') )
        wp_die('<p>'.__('You do not have sufficient permissions to edit templates for this site.','acn').'</p>');
    
    if ($content === null || $file_name === null){
        wp_die('<p>'.__('Error Downloading file.','acn').'</p>');     
    }
  ...
  public function create_export_download_link($echo = false){
    ...
    if ($echo === true)
        echo '<a href="'.$export_url.'" target="_blank">'.__('Download Export','acn').'</a>';
    elseif ($echo == 'url')
        return $export_url;
    return '<a class="button" href="'.$export_url.'" target="_blank">'.__('Download Export','acn').'</a>';
  }
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'acn', dirname(__FILE__) . '/lang/' . get_locale() .'.mo' );
  }
    ...
    foreach ($field['validate'] as $type => $args) {
      if (method_exists($this,'is_' . $type)){  
        if (call_user_func ( array( $this, 'is_' . $type ), $meta ,$args['param']) === false){
          ...
          $this->errors[$field['id']]['m'][] = (isset($args['message'])? $args['message'] : __('Not Valid ','acn') . $type);
          $ret = false;
        }
      }
    }
  ...
  public function displayErrors(){
    if ($this->errors_flag){
      ...
      echo '<h4>'.__('Errors in saving changes', 'acn').'</h4>';
  ...
  /**
   * Method to get an array of all types of icon in font-awesome
   * @access public
   * @author Enrique Crespo <contact@e-crespo.com>
   * @since 1.0.0
   * @param 
   * @return  array
   */
  public function getFontAwesomeIconList(){
    //Current Font Awesome version (4) begin all icons with 'fa-' . Change that if new version changes this or using
    //older version like 3, that begin all icons with 'icon-'
    $pattern = '/\.(fa-(?:\w+(?:-)?)+):before\s+{\s*content:\s*"(.+)";\s+}/';
    
    //Change source if new version available
    $subject = file_get_contents('http://netdna.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.css');
...
/**
* Auxiliar functions to make plugin compatible with PHP 5.2 old version
* array_replace_recursive() function Was used in the option panel to generate a list of post and pages to be linked in navigation elements
* that function is only available in PHP>=5.3
* 
* @since  1.1.1
* @access not part of the admin class
*/
function array_replace_recursive_php52($array, $array1)
{
    // handle the arguments, merge one by one
      ...
      for ($i = 1; $i < count($args); $i++)
      {
        if (is_array($args[$i]))
        {
          $array = recurse_php52($array, $args[$i]);
        }
      }
      return $array;
    }
    function recurse_php52($array, $array1)
    {
    foreach ($array1 as $key => $value)
    {
      ...
      if (is_array($value))
      {
        $value = recurse_php52($array[$key], $value);
      }
      $array[$key] = $value;
    }
    return $array;
