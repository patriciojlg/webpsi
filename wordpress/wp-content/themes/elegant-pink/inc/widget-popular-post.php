        ...
        if( $qry->have_posts() ){
            ...
            <ul>
                ...
                while( $qry->have_posts()){
                    $qry->the_post();
                    $img = wp_get_attachment_image_src( get_post_thumbnail_id(), 'elegant-pink-post-thumb', false );
                ?>
                    <li>
                        <?php if( has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail"><img src="<?php echo esc_url( $img[0] );?>" alt="<?php the_title();?>" /></a>
                        <?php }?>
						<header class="entry-header">
							<h3 class="entry-title"><a href="<?php the_permalink(); ?>"><?php the_title();?></a></h3>
							<?php if( $show_date ){?>
                                <span class="posted-on"><a href="<?php the_permalink(); ?>"><time datetime="<?php printf( __( '%1$s', 'elegant-pink' ), get_the_date('Y-m-d') ); ?>"><?php printf( __( '%1$s', 'elegant-pink' ), get_the_date('j F Y') ); ?></time></a></span>
                            <?php }?>
						</header>                        
                    </li>        
                <?php    
